...
May-2023: remove blcal, add applycal for the flux cal, update applycal modes (Koch)
...
from casatasks import (listobs, delmod, flagdata, setjy, bandpass, gaincal,
                       applycal, blcal, fluxscale, flagmanaager)
...
# Identify the scan number by looking in the CASA log from the listobs call above.
bpscan='1201'
...
gaincal(vis=vis,caltable =vis+'.bpself.gcal',
        field=bpcal,sp w=bpchans,refant=refant,scan=bpscan,
        calmode='p', solint='int', minsnr=2.0, minblperant=3)
...
gaincal(vis=vis, caltable=vis+'.scanphase_combinespw.gcal',
        field=calfields, refant=refant,
        combine='spw', spw='0~5',
        calmode='p', solint='inf', minsnr=2.0, minblperant=3,
        gaintable=[vis+'.bandpass.solnorm_true.bcal'])
# USB
...
gaincal(vis=vis, caltable=vis+'.amp.gcal',
        ...
        calmode='ap', solint='10min', minsnr=2.0, minblperant=3,
        gaintable=[vis+'.bandpass.solnorm_true.bcal',
...
fluxresults=fluxscale(vis=vis, caltable=vis+'.amp.gcal',
                      refspwmap=[-1],
                      transfer=[bpcal,bothpcal],
                      fluxtable=vis+'.flux.cal',
                      reference=flux)
print(fluxresults)
...
# To load, use:
# fluxresults = np.load(vis+".fluxscale.npy", allow_pickle=True).item()
...
# (list of 0s and 6s).
# NOTE: interp: for each field, this is how to average/interpolate the
# solutions in "time,freq"
applycal(vis=vis, field=bpcal,
         spw=spwrange,
         gaintable=[vis+'.bandpass.solnorm_true.bcal',
                    vis+'.intphase_combinespw.gcal',
                    vis+'.flux.cal'],
...
for this_pcal in bothpcal.split(","):
    ...
    applycal(vis=vis,field=this_pcal,
            ...
            gainfield=[bpcal, this_pcal, this_pcal],
            flagbackup=False, calwt=False)

# Flux calibrator:
applycal(vis=vis,field=flux,
         spw=spwrange,
            gaintable=[vis+'.bandpass.solnorm_true.bcal',
                       vis+'.intphase_combinespw.gcal',
                       vis+'.flux.cal'],
...
flagmanager(vis=vis, mode='save', versionname='afterapplycal')
