...
elif flux == pcal1 and pcal1 is not None:
    transfer_fields = [bpcal, pcal2]
elif flux == pcal2 and pcal2 is not None:
    transfer_fields = [bpcal, pcal1]
else:
    transfer_fields = [bpcal, bothpcal]

# Check we've caught every case:
orig_transfer_fields = copy(transfer_fields)
for ii in range(len(orig_transfer_fields)):
    this_field = transfer_fields[ii]
    if flux == this_field:
        transfer_fields.pop(ii)

transfer_fields = [this_field for this_field in transfer_fields if len(this_field) > 0]

if len(transfer_fields) > 0:
    ...
    print("There are no fields to bootstrap the flux calibrator model to")
    casalog.post("There are no fields to bootstrap the flux calibrator model to")
...
plotms(vis=gain_amp_scan_final_table,
       xaxis='freq', yaxis='amp',
       coloraxis='spw',iteraxis='antenna', ydatacolumn='data',
       xconnector='line', timeconnector=True,
...
try:
    ...
    print("Cannot find flux tables. Skipping")
    casalog.post("Cannot find flux tables. Skipping")
